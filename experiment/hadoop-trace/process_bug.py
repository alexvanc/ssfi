...
class Processor(object):
    ...
    def start(self):
        sql="select fault_id,error_file,activated from injection_record_hadoop where error_file is not null"
    ...
    def markBug(self,ID,error_files,activated):
        if os.path.exists(self.dataDir+"/"+ID):
            ...
            for error_file in file_list:
                ...
                if bug_type!=0:
                    sql="update injection_record_hadoop set with_bug=%s where fault_id='%s'" % (bug_type,ID)
                    self.cursor.execute(sql)
                    self.db.commit()
                    break
