        ...
        if self.IDActivated(ID):
            ...
            if os.path.exists(self.dataDir+'/fi/'+ID+'/'+'runResult.txt'):
                ...
                if len(lines)==0:
                    ...
                    if if ('Docker' in resultDict) and resultDict['Docker']=='1' and ('Hadoop' in resultDict) and resultDict['Hadoop']=='1' and ('Hello' in resultDict) and resultDict['Hello']=='2':
                        rightOutput=True
            else:
                crashed=True
            
            if timeResult['flag']=='ok' and timeResult['last_time']>240000:
                timeOut=True
            ...
            self.checkExceptionInLogs(ID,"logs")
        ...
        for dirpath,subdirs,files in os.walk(directory):
            for filename in files:
                if filename.endswith(".log"):
                    ...
                    tempFile.close()
                    if len(lines)!=0:
                        errorDict=self.parseALogFile(filename,dirpath,log_format2)
...
if __name__ == '__main__':
        processor=Processor("process_test")
        processor.start()
        processor.close()
