...
class LogParser:
    ...
    def new_parse(self,absoluteLogFilePath,log_format):
        ...
        for idx, line in df_log.iterrows():
            ...
            logmessageL =  re.split('[\s=:,_/\$]', self.preprocess(line['Content']))
    ...
    def parse_logfile(self,parentFolder,filename,log_format):
        ...
        structed_logfile.write("UUID,Tmpl_HASH,Level,Class,Date,Time,Tmpl\n")
        start_time = datetime.now()
        ...
        for idx, line in df_log.iterrows():
            ...
            # Add the new log message to the existing cluster
                ...
                template_id = hashlib.md5(template_str.encode('utf-8')).hexdigest()[0:8]
                structed_logfile.write(UUID+","+template_id+","+line['Level']+","+line.get("Class","log4j").replace("'",'"')+","+line.get("Date","2020-10-10")+","+line.get("Time","10:10:10.000").replace(",",".")+","+template_str+"\n")
        ...
        for lenList in self.rootNode.childD:
            ...
            for token in parentn.childD:
                if lenList==1:
                    ...
                    clusData['token']=token.replace("'",'"')
                    clusData['level']='INFO'
                    clusData['tmpl']=token.replace("'",'"')
                    clusData['hash']=hashlib.md5(token.encode('utf-8')).hexdigest()[0:8]
                    ...
                    for cluster in secondP.childD:
                        ...
                        clusData['tmpl']=' '.join(cluster.logTemplate).replace("'",'"')
                        clusData['hash']=hashlib.md5(' '.join(cluster.logTemplate).encode('utf-8')).hexdigest()[0:8]
