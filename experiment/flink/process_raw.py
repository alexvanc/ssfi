...
class Processor(object):
    ...
    def start(self,prefix):
        ...
        if self.prefix=='act':
            ...
            sql="select fault_id from injection_record_flink where activated=1"
    ...
    def processOneFI(self,fi,parser):
        ...
        if self.IDActivated(ID):
            ...
            if os.path.exists(self.dataDir+'/'+ID+'/'+'runResult.txt'):
                ...
                if len(lines)==0:
                    ...
                    if len(lines)>=len(self.outputDict):
                        try:
                            rightOutput=self.checkRightOutput(lines[-len(self.outputDict):])
                        except:
                            print(len(lines))
                            print(len(self.outputDict))
        ...
        sql="update injection_record_flink set failure_type='%s' where fault_id='%s'" % (failure_type,ID)
        try:
            self.cursor.execute(sql)
            self.db.commit()
        except:
    ...
    def checkRightOutput(self, lines):
        ...
        for line in lines:
            ...
            if len(kvp)!=2:
                ...
                resultDict[kvp[0][1:]]=int(kvp[1][:-1].strip())
    ...
    def checkExceptionInLogs(self,ID,logFolder,parser):
        ...
        log_format2 = '<Date> <Time> <Level> <Class> - <Content>'
        #template for stderr
        ...
        for dirpath,subdirs,files in os.walk(directory):
            for filename in files:
                ...
                elif filename.find("standalonesession")!=-1 or filename.find("taskexecutor")!=-1:
                    tempFile=open(os.path.join(dirpath,filename),'r')
                    lines=tempFile.readlines()
                    tempFile.close()
                    if len(lines)!=0:
        ...
        if resultDict["error_found"]:
            sql="update injection_record_flink set error_file='%s',error_class='%s' where fault_id='%s'" %(resultDict["error_file"],resultDict["error_class"].replace("'","`"),ID)
            try:
                self.cursor.execute(sql)
                self.db.commit()
            except:
    ...
    def getRunningTime(self,ID):
        ...
        sql="select running_time from injection_record_flink where fault_id='%s'" % ID
        self.cursor.execute(sql)
        results=self.cursor.fetchall()
    ...
    def IDActivated(self,ID):
        sql="select * from injection_record_flink where fault_id='%s' and activated=1" % ID
        self.cursor.execute(sql)
        results=self.cursor.fetchall()
        if len(results)==0:
            return False
