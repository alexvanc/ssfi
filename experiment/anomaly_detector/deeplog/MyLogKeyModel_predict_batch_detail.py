...
class Generator(object):
    ...
    def generatePredictData(self,subdir,template_space_size):
        ...
        for fi in allJobs:
            ...
            oneJobData.append([]) #related info for each input/output lables
            oneJobData.append(fi) #each job's 

            #str(template_id)+" "+UUID+" "+className+" "+line[dateIndex]+"_"+line[timeIndex]+" "+component_name+"\n")

            for dirpath,subdirs,files in os.walk(self.dataDir+"/"+subdir+"/"+fi):
                for tfile in files:
                    if tfile.endswith(".sequence"):
                        with open(os.path.join(dirpath,tfile),'r') as f:
                            tmplSequence=[]#for one log file
                            infoSequence=[]# for the info related with log
                            for line in f.readlines():
                                ...
                                if items[0]=="None":
                                    ...
                                    template_id=int(items[0])
                                if template_id>template_space_size:
                                            print("Error")
                                            print(os.path.join(dirpath,tfile))

                                tmplSequence.append(template_id)
                                infoSequence.append({'uuid':items[1],'class':items[-3], 'ltime':dt.datetime.strptime(items[-2].strip(),"%Y-%m-%d_%H:%M:%S.%f"), 'component':items[-1]})
                            ...
                            for i in range(len(tmplSequence) - window_size):
                                ...
                                info = infoSequence[i + window_size]
                                oneJobData[0].append(seq)
                                oneJobData[1].append(label)
                                oneJobData[2].append(info)
            ...
            for onePredict in predicted:
                if label[lineIndex] not in onePredict[-num_candidates:]:
                    if not found:
                        FP += 1
                        found=True
                        ftime=oneJobData[2][lineIndex]['ltime']
                        info=oneJobData[2][lineIndex]
                    else:
                        if oneJobData[2][lineIndex]['ltime']<ftime:
                            ftime=oneJobData[2][lineIndex]['ltime']
                            info=oneJobData[2][lineIndex]
                lineIndex=lineIndex+1
            if found:
                uploader.mark_deep_detail(oneJobData[3],info)
