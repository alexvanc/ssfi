...
class Processor(object):
    ...
    def start(self,prefix):
        ...
        if self.prefix=='act':
            ...
            sql="select fault_id from injection_record_elastic where activated=1"
    ...
    def processOneFI(self,fi,parser):
        ...
        if self.IDActivated(ID):
            ...
            if os.path.exists(self.dataDir+'/'+ID+'/'+'runResult.txt'):
                ...
                resultFile.close()
                if len(lines)==0 or content.strip()=='':
                    crashed=True
                else:
                    try:
        ...
        sql="update injection_record_elastic set failure_type='%s' where fault_id='%s'" % (failure_type,ID)
        try:
            self.cursor.execute(sql)
            self.db.commit()
        except:
    ...
    def checkRightOutput(self, content):
        jsonResult=json.loads(content)
        if jsonResult['_shards']['successful']!=0 and jsonResult['hits']['total']==4:
            return True
        return False
    ...
    def checkExceptionInLogs(self,ID,logFolder,parser):
        ...
        if os.path.exists(self.dataDir+'/'+ID+'/'+'runError.txt'):
                ...
                outputFile.close()
                if content.find("Exception")!=-1 or content.find("ERROR")!=-1:
                    resultDict['error_found']=True
        ...
        if resultDict["error_found"]:
            sql="update injection_record_elastic set error_file='%s',error_class='%s' where fault_id='%s'" %(resultDict["error_file"],resultDict["error_class"].replace("'","`"),ID)
            try:
                self.cursor.execute(sql)
                self.db.commit()
            except:
    ...
    def getRunningTime(self,ID):
        ...
        sql="select running_time from injection_record_elastic where fault_id='%s'" % ID
        self.cursor.execute(sql)
        results=self.cursor.fetchall()
    ...
    def IDActivated(self,ID):
        sql="select * from injection_record_elastic where fault_id='%s' and activated=1" % ID
        self.cursor.execute(sql)
        results=self.cursor.fetchall()
        if len(results)==0:
            return False
