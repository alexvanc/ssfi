...
#             print(value)
        ...
        insertTemp="insert into injection_record_spark(%s) values(%s)" % (tableColumns, valueTml)
    ...
    def parseFailure(self):
        ...
        if os.path.exists("/tmp/runError.txt"):
            resultFile=open("/tmp/runError.txt")
            lines=resultFile.readlines()
            resultFile.close()
            if len(lines)!=1:
                exceptions=True
        ...
        if rtime>240000:
            ...
            for dirpath,subdirs,files in os.walk(directory):
                for filename in files:
                    if filename.endswith(".log") or filename.endswith("audit") or filename=="syslog" or filename=="stdout":
