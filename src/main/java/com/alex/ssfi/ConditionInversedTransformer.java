...
public class ConditionInversedTransformer extends BasicTransformer {
    ...
    private void startToInject(Body b) {
        ...
        this.injectInfo.put("FaultType", "CONDITION_INVERSED_FAULT");// 标记故障类型为，"CONDITION_BORDER_FAULT"
        ...
        List<Stmt> allIfStmt = getAllIfStmt(b);//获取全部IF语句
        while (true) {
            int compStmtSize = allIfStmt.size();
            if (compStmtSize == 0) {//若待注入的IF语句数量为0，则返回上级程序
                break;
            }
            int randomCompIndex = new Random(System.currentTimeMillis()).nextInt(compStmtSize);// 随机选取目标IF语句的索引
            Stmt targetCompStmt = allIfStmt.get(randomCompIndex);// 获取目标IF语句
    ...
    private boolean inject(Body b, Stmt stmt) {
        ...
        try {
            ...
            if (expr instanceof GtExpr) {//若expr为>表达式
                ...
                LeExpr leExp = Jimple.v().newLeExpr(gtExp.getOp1(), gtExp.getOp2());//创建Le表达式
                ...
                LtExpr ltExp = Jimple.v().newLtExpr(geExp.getOp1(), geExp.getOp2());//创建Lt表达式
                ...
                GeExpr geExp = Jimple.v().newGeExpr(ltExp.getOp1(), ltExp.getOp2());//创建Ge表达式
                ...
                GtExpr gtExp = Jimple.v().newGtExpr(leExp.getOp1(), leExp.getOp2());//创建Gt表达式
                ...
                return true;
            } else if (expr instanceof EqExpr) {//若expr为==表达式
                EqExpr eqExp = (EqExpr) expr;
                NeExpr neExp = Jimple.v().newNeExpr(eqExp.getOp1(), eqExp.getOp2());//创建Ne表达式
                newIfStmt.setCondition(neExp);//设置新的IF语句条件
                units.insertBefore(newIfStmt, ifStmt);// 在原ifStmt前插入新的IF语句
                ...
                return true;
            } else if (expr instanceof NeExpr) {//若expr为!=表达式
                NeExpr neExp = (NeExpr) expr;
                EqExpr eqExp = Jimple.v().newEqExpr(neExp.getOp1(), neExp.getOp2());//创建Eq表达式
                newIfStmt.setCondition(eqExp);//设置新的IF语句条件
                units.insertBefore(newIfStmt, ifStmt);// 在原ifStmt前插入新的IF语句
